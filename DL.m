function cr = DL(A,b,c,epsilon)
    ...
    learn_t=round(2.^(1:floor(log(1/epsilon)/log(2)))*epsilon*n_time);
    for t=n_learn+1:n_time
        x_hat(t)= c(t)>p_hat'*A(:,t);
        if any(A(:,t)*x_hat(t)>b-A(:,1:t-1)*x_hat(1:t-1))
            x_hat(t)=0;
        end
        if any(t==learn_t)% if it is time to learn
            P_hat=linprog([b*t/n_time*(1-epsilon*sqrt(n_time/t));ones(t,1)],-[A(:,1:t)' eye(t)],-c(1:t),[],[],zeros(n_const+t,1),[],options);
            p_hat=P_hat(1:n_const);
        end
    end
    cr=(c'*x_hat)/(c'*x);
